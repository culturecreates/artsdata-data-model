...
Dans Artsdata, un événement est défini comme « une activité organisée qui se déroule à un moment et un lieu précis ». La classe [`adr:Event`](http://kg.artsdata.ca/ontology/Event) est considérée comme une classe équivalente à [`schema:Event`](https://schema.org/Event). 
...
Dans vos données structurées, les événements peuvent être classés à l’aide de types d’événements supplémentaires provenant du [Vocabulaire contrôlé Artsdata pour les types d’événements](../event-types.fr.html), en utilisant la propriété `additionalType`.

Outre les concepts issus du vocabulaire contrôlé Artsdata pour les types d’événements, Artsdata accepte également les sous-types d’événements Schema.org ainsi que les concepts Wikidata.

Une fois vos données publiées chargées dans Artsdata, des types d’événements Artsdata seront automatiquement attribués à vos événements au moyen de la propriété `additionalType`.

## Exigences minimales pour la création d'identifiants Artsdata pour les événements

Pour qu’Artsdata puisse automatiquement créer un identifiant unique et pérenne pour un événement ou associer un événement à un identifiant existant, l’événement doit au minimum comporter les paires attribut-valeur suivantes :

- `@Type`: `Event` (ou un sous-type d'événement);
- `name`;
- `startTime`: valeur de date et d’heure au format ISO 8601 (avec ou sans décalage de fuseau horaire);
- `location`: objet `Place` comportant un `name` et des renseignements géographiques conformes, OU un lien [`sameAs`]({{ base }}/identifiers-guidelines/sameas.fr.html) vers un URI Wikidata ou Artsdata identifiant le lieu.

Si un événement externe possède le même `location.address.postalCode` qu’un événement auquel Artsdata a déjà attribué un identifiant, cet événement est signalé comme doublon potentiel afin d’éviter la création d’un identifiant Artsdata en double. Un événement signalé comme doublon potentiel doit être examiné par une personne responsable de l’intendance des données Artsdata avant qu’une décision soit prise quant à la création d’un identifiant ou à l’association à un identifiant existant. Le signalement des doublons potentiels ne s’applique que lorsque l’objet `Event.location` n’a pas encore été désambiguïsé. Si le lieu d’un événement a déjà été associé manuellement à une entité `Place` dans le graphe central Artsdata par une personne responsable de l’intendance des données Artsdata, ce lien `sameAs` est considéré comme fiable et aucun doublon potentiel n’est signalé, même si les codes postaux correspondent.

Si un objet `Event` ne satisfait pas entièrement à ces exigences, il est toujours possible d'attribuer manuellement un identifiant, à condition qu'au moins l'une des propriétés suivantes soit renseignée :
- `description`;
- `url`.
...
| [location](http://schema.org/location) | Requis | Saisissez le lieu où se déroule l’événement. Dans le cas d’un lieu physique, la valeur devrait être aussi précise que possible (par exemple, la salle de spectacle à l’intérieur du bâtiment plutôt que le bâtiment lui-même). À des fins de désambiguïsation et de réconciliation, il est fortement recommandé d’attribuer à la propriété `sameAs` un lien vers l’URI Wikidata ou Artsdata du lieu. Sinon, une entité `PostalAddress` complète, comprenant le code postal et les abréviations postales à deux lettres de la province ou du territoire et du pays, peut faciliter la désambiguïsation. Pour plus d’informations, consultez ces [lignes directrices]({{ base }}/location.fr.html) |
...
| [organizer](http://schema.org/organizer) | Recommandé | Saisissez les renseignements permettant d’identifier et de décrire l’organisme ou la personne qui présente l’événement. Dans le cas d’un organisme, il peut s’agir d’un organisme diffuseur ou même d’une compagnie présentant sa propre production, auquel cas la même valeur serait saisie sous la propriété `performer`. Les propriétés recommandées pour l’entité imbriquée incluent : `name`, `url` et `sameAs` (avec un lien pointant vers une URI Wikidata ou Artsdata). |
| [performer](http://schema.org/performer) | Recommandé | Saisissez les renseignements permettant d’identifier et de décrire la compagnie, le groupe ou le ou les artistes principaux qui assurent la représentation. Les propriétés recommandées pour l’entité imbriquée incluent : `name`, URL et `sameAs` (avec un lien pointant vers une URI ISNI, Wikidata ou Artsdata). Pour plus d'informations, consultez ces [lignes directrices]({{ base }}/performer.fr.html). |
| [workPerformed](http://schema.org/workPerformed) | Facultatif | Saisissez les informations identifiant et décrivant l'œuvre-spectacle exécutée lors de la représentation ou alors décrivant la ou des œuvres musicales, dramatiques, musicodramatiques intégrées à l'œuvre-spectacle. Les propriétés recommandées pour l'entité imbriquée incluent : `name`, `url`, `genre`, `inLanguage` et `sameAs` (pointant vers une URI Artsdata ou Wikidata). Si l’œuvre elle-même ne possède pas d’URI Wikidata ou Artsdata, une entité `creator` imbriquée peut être ajoutée en indiquant le `name` du créateur et son `sameAs` (avec un lien pointant vers une URI Wikidata ou Artsdata).|
